...
# 更新状态栏
refresh() {
    _icons='';_music='';_wifi='';_cpu='';_mem='';_date='';_vol='';_bat='';_brightness=# 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_brightness$_music$_wifi$_cpu$_mem$_date$_vol$_bat"      # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 300)) -eq 0 ] && to=(${to[@]} bat brightness)                      # 每 300秒 更新 bat
...
case $1 in
    ...
    updateall|check) update brightness icons music wifi cpu mem date vol bat ;;
