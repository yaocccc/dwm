...
# 更新状态栏
refresh() {
    _icons=''; _wifi=''; _cpu=''; _mem=''; _date=''; _vol=''; _brightness=''; _bat='';   # 重置所有模块的状态为空
    source $tempfile                                                     # 从 temp 文件中读取模块的状态
    xsetroot -name "$_icons$_wifi$_cpu$_mem$_date$_vol$_brightness$_bat"             # 更新状态栏
}
...
cron() {
    ...
    while true; do
        ...
        [ $((i % 20)) -eq 0 ]  && to=(${to[@]} cpu mem vol brightness icons)        # 每 20秒  更新 cpu mem vol icons
...
# cron 启动定时更新状态栏
# update 更新指定模块 `update cpu` `update mem` `update date` `update vol` `update brightness` `update bat` 等
# updateall 更新所有模块 | check 检查模块是否正常(行为等于updateall)
# * 处理状态栏点击 `cpu 按键` `mem 按键` `date 按键` `vol 按键` `brightness 按键` `bat 按键` 等
case $1 in
    cron) cron ;;
    update) shift 1; update $* ;;
    updateall|check) update icons wifi cpu mem date vol brightness bat ;;
